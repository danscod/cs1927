...
void addLink(Map g, Location start, Location end, Transport type)
{
    ...
    assert(start >= 0 && start <= g->nV); 
    assert(end >= 0 && end <= g->nV); 
    assert(start != end);

    // increment the number of edges in the graph
    g->nE++;
    
    // create a new edge node (the forwards direction: start -> end)
    VList new = malloc(sizeof(struct vNode));
    assert(new != NULL);
    new->type = type;
    new->v = end;
    // case if there are no nodes in the list yet
    if (g->connections[start] == NULL) 
    {
        g->connections[start] = new;
        new->next = NULL;
    } 
    // case if there are one or more nodes (add to start of list)
    else
    {
        new->next = g->connections[start];
        g->connections[start] = new;

    }

    // create a new edge node (the backwards direction: end -> start)
    VList new1 = malloc(sizeof(struct vNode));
    assert(new1 != NULL);
    new1->type = type;
    new1->v = start;
    // case if there are no nodes in the list yet
    ...
    {
        g->connections[end] = new1;
        new1->next = NULL;
    } 
    // case if there are one or more nodes (add to start of list)
    else
    {
        new1->next = g->connections[end];
        g->connections[end] = new1;
...
int connections(Map g, Location start, Location end, Transport type[])
{
    ...
    // navigate through the array to desired list
    ...
    while(curr != NULL)
    {
        // each time the connection matches, add the type to type[]
        if(curr->v == end) 
        {
            type[i] = curr->type;
            i++;
    ...
    // return a count of how many connections
    return i; 
}
