...
{
	//Case if root (n1) is nonexistent
	if (n1 == NULL) {
		return NULL;
	}

	//Case if no left node exists
	Link n2 = n1->left;
	if (n2 == NULL) {
		n1->within->nrotates = n1->within->nrotates + 1; //Rotation counter
		return n1;
	}

	//Case if left node does exist
	n1->left = n2->right;
	n2->right = n1;
	n1->within->nrotates = n1->within->nrotates + 1; //Rotation counter
	return n2;
...
{
	//Case if root (n2) is nonexistent
	if (n2 == NULL) {
		return NULL;
	}

	//Case if no right node exists
	Link n1 = n2->right;
	if (n1 == NULL) {
		n2->within->nrotates = n2->within->nrotates + 1; //Rotation counter
		return n2;
	}

	//Case if right node does exist
	n2->right = n1->left;
	n1->left = n2;
	n2->within->nrotates = n2->within->nrotates + 1; //Rotation counter
	return n1;
...
// Converted cmp macro to function
int cmp(Item k1, Item k2) 
{
	thisTree->ncompares = thisTree->ncompares + 1; //Comparison counter
	return ((k1) - (k2));
}
